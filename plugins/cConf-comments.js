...
Draw.loadPlugin(function(ui)
{
	...
	function confCommentToDrawio(atlasComment, parentId, siteUrl)
	{
		...
		var comment = new DrawioComment({attVer: atlasComment.attVer, ui: ui}, atlasComment.id, 
				...
				atlasComment.version.createdAt, atlasComment.version.createdAt, false,
		...
		if (atlasComment.valuePrefex)
		{
			comment.valuePrefex = atlasComment.valuePrefex;
		}
		...
		var replies = atlasComment.children || [];
			...
			ui.remoteInvoke('getComments', [null, false], null, function(comments, siteUrl)
			{
				...
				for (var i = 0; i < comments.length; i++)
				{
					...
					if (cnt.indexOf(REPLY_MARKER) == 0)
					{
						...
						comments[i].body.storage.value = encodeURIComponent(cnt.substring(REPLY_MARKER_END.length + end));
						comments[i].valuePrefex = cnt.substring(0, REPLY_MARKER_END.length + end);
				...
				for (var i = 0; i < oldVerReplies.length; i++)
				{
					...
					if (pComment != null)
					{
						...
						pComment.children.push(oldVerReplies[i].reply);
					}
				}
	...
	DrawioComment.prototype.editComment = function(newContent, success, error)
	{
		...
		else if (commentsVer == 2)
		{
			...
			ui.remoteInvoke('editComment', [this.id, this.version, (this.valuePrefex? this.valuePrefex : '') + newContent], null, function(version)
			{
