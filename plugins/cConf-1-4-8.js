...
{
	var MAX_DIAGRAMS_NO_LICENSE = 10;
	var licenseValid = null, totalDiagramCount = null, isNewDiagram = null;

	function checkFreeLimitReached()
	{
		if (isNewDiagram && licenseValid === false && totalDiagramCount > MAX_DIAGRAMS_NO_LICENSE)
		{
			ui.remoteInvoke('showNotification', [{
				title: MAX_DIAGRAMS_NO_LICENSE + '+ Diagrams',
				body: mxResources.get('licenseRequired'),
				type: 'error',
				close: 'manual'
			}, true], null, function(){}, function(){});
			
			logInfo('ConfNoLicense:CountExceeded:' + totalDiagramCount, xdm_e);
		}
	};

	function logInfo(message, url)
	{
		try
		{
			var img = new Image();
	...
	//Remote invoke is not ready yet and no access to AC so code is repeated here
	function setupConfPlaceholders(graph, info, macroData)
	{
		...
		graph.getGlobalVariable = function(name)
		{
			try
			{
				...
				{
					return macroData.diagramDisplayName || macroData.diagramName || info.title;
				}
	...
	var macroData = {}, unpublishedChanges = false;
	...
	mxEvent.addListener(window, 'message', mxUtils.bind(this, function(evt)
	{
		...
		try
		{
			...
			if (data.action == 'load')
			{
				...
				//Remote invoke is not ready yet and no access to AC so code is repeated here
				setupConfPlaceholders(ui.editor.graph, data.attInfo, macroData);
				//Fetch notifications
	...
	ui.remoteInvoke('checkConfLicense', [license, xdm_e], null, function(licenseValid_p)
	{
	    ...
	    if (!licenseValid)
	    {
			...
			if (typeof(MathJax) !== 'undefined')
			{
				ui.actions.get('mathematicalTypesetting').funct = function()
				{
					...
					logInfo('ConfNoLicense::Math', xdm_e);
				};
			}
			...
			EditorUi.prototype.insertPage = function(page, index)
			{
				...
				logInfo('ConfNoLicense::Page', xdm_e);
			};
			...
			ui.importFiles = function() 
			{
				...
				ui.alert(mxResources.get('licenseRequired'));
				logInfo('ConfNoLicense::Import', xdm_e);	
			}
			...
			ui.getComments = function(success, error)
			{
				...
				logInfo('ConfNoLicense:Comment', xdm_e);
			}
			...
			// Check macros count and close editor if exceeding (MAX_DIAGRAMS_NO_LICENSE)
			ui.remoteInvoke('countDrawioMacros', null, null, function(count)
			{
				totalDiagramCount = count;
				checkFreeLimitReached();
			},
			function(){});
	    }
	...
	DiagramFormatPanel.prototype.addViewerOptions = function(div)
	{
		...
		}, function(checked)
		{
			...
			ui.getCurrentFile().wasModified = true;
		}));
	...
	LinkDialog = function(editorUi, initialValue, btnLabel, fn, showPages)
	{
		...
		ui.remoteInvoke('getCurPageAttachments', null, null, function(atts)
		{
			...
			else
			{
				atts = atts.filter(function(a)
				{
					...
					return a.mediaType != 'application/vnd.jgraph.mxfile.cached' && !/^\~.+\.tmp$/.test(a.title); 
				});
		...
		mxEvent.addListener(attSelect, 'change', function()
		{
			...
			if (att.mediaType == 'application/vnd.jgraph.mxfile')
			{
				...
				var pageId = att.pageId || att.blogPostId;
		...
		if (window.location.hostname === 'wbc.draw.io')
		{
			typesMap = {
				'page': mxResources.get('page'),
				'attachment': mxResources.get('attachment', null, 'Attachment'),
				'blogpost': mxResources.get('blog'),
				'ac:com.jgraph.confluence.plugins.whiteboards:drawio-whiteboard': mxResources.get('drawDiag')
			};
		}
		else
		{
	...
	// Note: Same code as in viewer but we don't have access to AC here (TODO find a solution)
	function processLink(url)
	{
		if (Editor.config != null && Editor.config.linkAdjustments)
		{
			var linkAdjustments = Editor.config.linkAdjustments;
	...
	{
		return graphCreateLinkForHint.call(this, processLink(link), label, associatedCell);
	}
	//======================== Revisions ========================
	...
	ui.getRevisions = function(success, error)
	{
		...
		{
			ui.remoteInvoke('getFileContentAsIs', [this.downloadUrl], null, success, error);
		};
	...
	function addImage(container, data, w, h, img) 
	{
		...
		{
			var cells = ui.stringToCells((img.xml.charAt(0) == '<') ?
		...
		recentDocsCallback, searchDocsCallback, openExtDocCallback, showImport, createButtonLabel, customTempCallback, withoutType)
	{
			...
			searchDocsCallback == null && openExtDocCallback == null && customTempCallback == null)
		{
			...
			{
				ui.remoteInvoke('getFileContentAsIs', [url], null, callback, function()
				{
		...
		// Change default file name to be unique by using timestamp
		editorUi.defaultFilename = mxResources.get('untitledDiagram') + '-' + Date.now();
	...
	mxSvgCanvas2D.prototype.image = function (x, y, w, h, src, aspect, flipH, flipV, clipPath)
	{
		...
		if (baseUrl && !cachedImg && src.substring(0, baseUrl.length) == baseUrl)
		{
			ui.remoteInvoke('fetchConfluenceImage', [src, baseUrl], null, function(base64Img)
			{
				mxSvgCanvas2D.confImagesCash[src] = base64Img;
				origImage.call(that, x, y, w, h, base64Img, aspect, flipH, flipV, clipPath);
			},
			function(){}); // Keep the image blank
		}
		else
		{
	...
	//=============Adding Editing Support for Confluence Libraries==============================
	var libsPageId = null;
	/**
	 * 
	 */
	...
	RemoteLibrary.prototype.save = function(revision, success, error)
	{
		...
		function checkName()
		{
			ui.remoteInvoke('getPageAttachments', [libsPageId], null, function(atts)
			{
				for (var i = 0; i < atts.length; i++)
				{
					if (atts[i].title.toLowerCase() == _this.libObj.title.toLowerCase())
					{
						error({code: 409, message: mxResources.get('confALibExist')});
						return;
					}
				}
		...
		function doSave(noNameCheck)
		{
			if (!noNameCheck && _this.libObj.id == null)
			{
				checkName();
				return;
			}

			ui.remoteInvoke('createAttFile', [libsPageId, _this.libObj.title, 'text/xml', _this.data], null, function(resp)
			{
				resp = JSON.parse(resp).results[0];
				
				// Update new libraries only since changing the ID of existing libraries duplicate it due to having the version as part of the ID
				if (_this.libObj.id == null)
				{
					_this.libObj = {id: resp.id, title: resp.title, downloadUrl: resp._links.download};
				}
			...
			}, function(err) 
			{
				if (err.status == 403)
				{
					error({code: err.status, message: mxResources.get('saveLibrary403')});
				}
				else
				{
					error({code: err.status, message: JSON.parse(err.responseText).message});
				}
			});
		}
		...
		{
			ui.remoteInvoke('getCustomLibrariesPages', [{}], null, function(pages)
			{
				if (pages.length == 0)
				{
					error({code: 404, message: mxResources.get('drawConfigNotFoundInst')});
				}
				else
				{
					libsPageId = pages[0].pageId;
					doSave();
				}
			}, error);
		}
	...
	// New library dialog
	var origSaveLibrary = ui.saveLibrary;

	ui.saveLibrary = function(name, images, file, mode, noSpin, noReload, fn)
	{
		if (file == null && mode == 'CONF_LIB')
		{
			file = new RemoteLibrary(this, this.createLibraryDataFromImages(images), {
				title: name.indexOf('.xml') == name.length - 4 ? name : name + '.xml'
			});
		}
		
		origSaveLibrary.call(this, name, images, file, mode, noSpin, noReload, fn);
	};
	...
	EmbedFile.prototype.setModified = function(value)
	{
		...
		this.wasModified = this.wasModified || value; // Mark the file as modified during this session
	};
								...
								try
								{
									...
									if (resp != false)
									{
										...
										if (token != null)
										{
											...
											}), success, token); // In case of error, the file is already saved (TODO Is this correct?)
										}
										else
										{
											success(resp);
	...
	*/
	...
	ui.actions.get('save').funct = function(exit)
	{
		...
		var isNewFile = desc == null || (desc.key == null && !desc.hasDraft);
		...
		function doActions()
		{
			if (isNewFile || curFile.wasModified || unpublishedChanges)
			{
			...
			{
				curFile.sync.descriptorChanged(
					curFile.getCurrentRevisionId());
			}
		};
